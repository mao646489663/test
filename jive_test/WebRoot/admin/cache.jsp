...
<%    // Permission check
    ...
    if (clearCache) {
        for (int i=0; i<cacheIDs.length; i++) {
            if (cacheIDs[i] != -1) {
                com.jivesoftware.util.Cache cache = caches[cacheIDs[i]];
                cache.clear();
            }
        }
        // Done so redirect
...
<script language="JavaScript" type="text/javascript">
function selAll() {
    var cb = eval('document.cacheForm.cacheID');
...
<%  if (cacheEnabled) { %>
<form action="cache.jsp" method="post" name="cacheForm">
...
<%    } %>
    <tr bgcolor="#ffffff">
        ...
        <td align="center">
            <font size="-2" face="verdana">
            [<a href="#" onclick="return selAll();">Select All</a>]<br>
            </font>

...
<%  if (threadIterator != null && threadIterator.hasPrevious()) {
        ...
        threadIterator.next();
        String subj = StringUtils.escapeHTMLTags(prevThread.getRootMessage().getUnfilteredSubject());
        // Replace any " in the subject
        subj = StringUtils.replace(subj, "\"", "&quot;");
%>
    <a href="forumContent_thread.jsp?forum=<%= forumID %>&thread=<%= prevThread.getID() %>&tstart=<%=threadStart %>&trange=<%= threadRange %>"
...
<%  if (threadIterator != null && threadIterator.hasNext()) {
        ...
        // reverse the iterator pointer back to the original index
        threadIterator.previous();
        String subj = StringUtils.escapeHTMLTags(nextThread.getRootMessage().getUnfilteredSubject());
        // Replace any " in the subject
        subj = StringUtils.replace(subj, "\"", "&quot;");
%>
    <font size="-1">
    <a href="forumContent_thread.jsp?forum=<%= forumID %>&thread=<%= nextThread.getID() %>&tstart=<%=threadStart %>&trange=<%= threadRange %>"
...
<tr>
    <td bgcolor="<%= (isRootMessage)?"#dddddd":"#eeeeee" %>">
        ...
        <tr>
            ...
            Posted By: <i>Guest</i>
            <%  } %>
            on <%= SkinUtils.formatDate(request,pageUser,theMessage.getCreationDate()) %>
            </font>
            </td>
        	<td width="1%" nowrap>
                <table cellpadding="2" cellspacing="0" border="0">
...
<%      if (isRootMessage && numReplies > 0) { %>
    <font size="-1"><b>Replies:</b> (<%= numReplies %> total)</font><p>
<%      } %>
...
%>
    <a href="forumContent_thread.jsp?forum=<%= forumID %>&thread=<%= prevThread.getID() %>&tstart=<%=threadStart %>&trange=<%= threadRange %>"
     title="Thread: <%= subj %>"><img src="images/prev.gif" width="10" height="10" hspace="2" alt="Thread: <%= subj %>" border="0"></a>
    <font size="-1">
    <a href="forumContent_thread.jsp?forum=<%= forumID %>&thread=<%= prevThread.getID() %>&tstart=<%=threadStart %>&trange=<%= threadRange %>"
     title="Thread: <%= subj %>">Previous Thread</a>
    </font>
...
%>
    ...
    <a href="forumContent_thread.jsp?forum=<%= forumID %>&thread=<%= nextThread.getID() %>&tstart=<%=threadStart %>&trange=<%= threadRange %>"
     title="Thread: <%= subj %>"><img src="images/next.gif" width="10" height="10" hspace="2" alt="Thread: <%= subj %>" border="0"></a>
<%  } else { %>
    &nbsp;
<%  } %>
    </td>

    ...
    <td width="99%" bgcolor="#eeeeee" valign="top">
        ...
        <tr>
            ...
            <%  }
                ...
                // we need to determine whether to show the move icon or not.
                boolean isMultipleCatAdmin = false;
                if (category.isAuthorized(Permissions.SYSTEM_ADMIN | ForumPermissions.FORUM_CATEGORY_ADMIN)) {
                    if (category.isAuthorized(Permissions.SYSTEM_ADMIN)) {
                        isMultipleCatAdmin = true;
                    }
                    else {
                        List cats = categoriesWithPermission(forumFactory, Permissions.SYSTEM_ADMIN | ForumPermissions.FORUM_CATEGORY_ADMIN);
                        for (int i = 0; i < cats.size(); i++) {
                            ForumCategory c = (ForumCategory) cats.get(i);

                            if (c.getID() == category.getID()) {
                                continue;
                            }

                            Iterator subCats = category.getRecursiveCategories();
                            boolean found = false;
                            while (subCats.hasNext()) {
                                ForumCategory subCat = (ForumCategory) subCats.next();
                                if (subCat.getID() == c.getID()) {
                                    found = true;
                                    break;
                                }
                            }

                            // if the category is not a subcategory then it must be another category
                            if (!found) {
                                isMultipleCatAdmin = true;
                            }
                        }
                ...
                while (forums.hasNext()) {
                    ...
                    boolean hasForumEditPerms = forum.isAuthorized(Permissions.SYSTEM_ADMIN | ForumPermissions.FORUM_CATEGORY_ADMIN | ForumPermissions.FORUM_ADMIN | ForumPermissions.MODERATOR);
                    boolean hasForumAdminPerms = forum.isAuthorized(Permissions.SYSTEM_ADMIN | ForumPermissions.FORUM_CATEGORY_ADMIN | ForumPermissions.FORUM_ADMIN);
                    boolean hasCatAdminPerms = forum.isAuthorized(Permissions.SYSTEM_ADMIN | ForumPermissions.FORUM_CATEGORY_ADMIN);
            %>
                <tr bgcolor="#ffffff">
                    <td>
                        <% if (hasForumAdminPerms) { %>
                            <font size="-1" face="arial">
                            <a href="editForum.jsp?forum=<%= forum.getID() %>"
                             ><b><%= forum.getName() %></b></a>
                            </font>
                        <%  } else { %>
                            <font size="-1" face="arial"><b><%= forum.getName() %></b></font>
                        <%  } %>
                    ...
                    <tr><td>
                    <%  if (forumIndex > 0 && hasCatAdminPerms) { %>
                    ...
                    <td>
                    <%  if (forums.hasNext() && hasCatAdminPerms) { %>
                    ...
                    <td align="center">
                        <%  if (hasCatAdminPerms && isMultipleCatAdmin) { %>
                            <font size="-1" face="arial">
            ...
            <%  if (hadForums) { %>
                <tr bgcolor="#ffffff">
                    ...
                    <td align="center">
                        <%  if (category.isAuthorized(Permissions.SYSTEM_ADMIN | ForumPermissions.FORUM_CATEGORY_ADMIN) && isMultipleCatAdmin) { %>
                            <font size="-1" face="arial">
        ...
        <tr>
            ...
            <%  }
                ...
                while (categories.hasNext()) {
                    ...
                    boolean hasCatAdminPerms = subCat.isAuthorized(Permissions.SYSTEM_ADMIN | ForumPermissions.FORUM_CATEGORY_ADMIN);
                    boolean hasParentCatAdminPerms = category.isAuthorized(Permissions.SYSTEM_ADMIN | ForumPermissions.FORUM_CATEGORY_ADMIN);
            %>
                <tr bgcolor="#ffffff">
                    <td>
                        <font size="-1" face="arial">
                    ...
                    <td align="center">
                        ...
                        <tr>
                            <td>
                            <%  if (catIndex > 0 && hasParentCatAdminPerms) { %>
                            ...
                            <td>
                            <%  if (categories.hasNext() && hasParentCatAdminPerms) { %>

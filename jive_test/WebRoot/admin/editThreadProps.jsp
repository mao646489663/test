...
<%  // Title of this page and breadcrumbs
    String title = "Thread Properties";
...
Edit the extended properties of a thread using the form below.
...
<%  }
    while (properties.hasNext()) {
        ...
        String pValue = thread.getProperty(pName);
%>
    <tr bgcolor="#ffffff">
        <td><%= pName %></td>
        <td><%= pValue %></td>
        <td align="center"><a href="editThreadProps.jsp?forum=<%= forumID %>&thread=<%= threadID %>&edit=true&propName=<%= pName %>"
            ><img src="images/button_edit.gif" width="17" height="17" alt="Click to edit this property" border="0"></a
            >
        </td>
        <td align="center"><a href="editThreadProps.jsp?forum=<%= forumID %>&thread=<%= threadID %>&delete=true&propName=<%= pName %>"
...
<%  if (edit) { %>
    <td><textarea cols="40" rows="10" name="propValue" wrap="virtual"><%= thread.getProperty(propertyName) %></textarea></td>
<%  } else { %>

...
<%  // Title of this page and breadcrumbs
    ...
    if (showForumList && !isSystemAdmin && !isCatAdmin && !isForumAdmin && isModerator) {
...
<%  if (showForumList) { %>
    ...
    <tr bgcolor="#ffffff">
        <td width="96%">
        <%  if (canEdit) { %>
            <font size="-1"><b><a href="forumContent.jsp?forum=<%= f.getID() %>"><%= f.getName() %></a></b></font>
        <%  } else { %>
            <font size="-1"><b><%= f.getName() %></b></font>
        <%  } %>

            <%  if (description != null) { %>
            <font size="-2"><br><%= f.getDescription() %></font>
            <%  } %>
        </td>
        <td width="1%" align="center"><font size="-2"><%= f.getForumCategory().getName() %></font></td>
        <td width="1%" align="center"><font size="-1"><%= LocaleUtils.getLocalizedNumber(threadCount,JiveGlobals.getLocale()) %></font></td>
        <td width="1%" align="center"><font size="-1"><%= LocaleUtils.getLocalizedNumber(messageCount,JiveGlobals.getLocale()) %></font></td>
        <td width="1%" nowrap><font size="-1"><%= SkinUtils.formatDate(request,pageUser,f.getModificationDate()) %></font></td>
    </tr>
    <%  } %>
    </table>
    </td></tr>
...
<%  } else { // showThreadList %>
    ...
    <%  } else { %>
        <font size="-1">
        <a href="forumContent.jsp"><b>Forum List</b></a> <b>&raquo;</b> <b><%= forum.getName() %></b>
        </font>
    <%   } %>
    ...
    <%  } else { %>
        ...
        <tr bgcolor="#ffffff" id="r<%= id %>">
            <td>
                ...
                <%  } %>
                <font size="-1"><b><a href="forumContent_thread.jsp?tstart=<%= start %>&trange=<%= range %>&forum=<%=forumID%>&thread=<%= theThread.getID() %>"><%= theThread.getName() %></a></b></font></td>
            <td align="center"><font size="-1">
                <%= LocaleUtils.getLocalizedNumber(theThread.getMessageCount()-1,JiveGlobals.getLocale()) %></font>
            </td>
            <td>
            <%  if (author != null) { %>
                <font size="-1"><a href="userProfile.jsp?user=<%= author.getID() %>"
                 title="<%= (name!=null)?name:"" %>"><%= author.getUsername() %></a></font>
            <%  } else { %>
                <font size="-1"><i>Guest</i></font>
            <%  } %>
            </td>
            <td nowrap><font size="-1"><%= SkinUtils.formatDate(request,pageUser,theThread.getModificationDate()) %></font></td>
            <td align="center"

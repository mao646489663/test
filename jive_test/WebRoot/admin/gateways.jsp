...
<%	// get parameters
    ...
    boolean newIsCutOffDateDisabled = ParamUtils.getBooleanParameter(request, "disableCutOffDate", false);
    ...
    boolean isCutOffDateDisabled = gatewayManager.isDisableCutOffDate();
    ...
    if (doSetGlobalSettings) {
        ...
        if (gatewayCount > 0 && newIsImportEnabled || newIsExportEnabled) {
            for (int i = 0; i < gatewayCount; i++) {
                ...
                if (gateway instanceof ImapGateway) {
                    ImapImporter importer = (ImapImporter) gateway.getGatewayImporter();
                ...
                else if (!errors && gateway instanceof EmailGateway) {
        ...
        if (!errors) {
            ...
            if (newIsCutOffDateDisabled != isCutOffDateDisabled) {
                gatewayManager.setDisableCutOffDate(newIsCutOffDateDisabled);
            }
...
<%  // Check for a one time message
    ...
    if (message != null) {
%>
    <font size="-1" color="#006600"><i><%= message %></i></font>
    <p>
<%  }
    ...
    <tr bgcolor="#ffffff">
        <%
            ...
            if (isEmailGateway) {
                ...
                .getToAddress();
                if (displayName == null) {
                    displayName = emailGateway.getSMTPHost();
                    if (displayName == null) {
                        displayName = emailGateway.getPOP3Host();
                    }
                }
                */
                if (gateway instanceof ImapGateway) {
        ...
        <%  if (isEmailGateway) { %>
            ...
            ><img src="images/button_edit.gif" width="17" height="17" alt="Edit the properties of this gateway" border="0"
            ></a>
        <%  } else if (isNewsgroupGateway) { %>
...
<ul>
    ...
    <tr>
    	...
    	<td><font size="-1"><label for="rb01">Email Gateway -- Synchronizes the forum with an email account or mailing list.</label></font></td>
    </tr>
    <%  } %>
    ...
    <tr>
    	...
    	<td><font size="-1"><label for="rb03">Newsgroup Gateway -- Synchronizes the forum with a NNTP newsgroup.</label></font></td>
    </tr>
    <%  } %>
    <tr>
...
<ul>
    ...
    <tr>
    	...
    	<td>
            <font size="-1">
            <input type="radio" name="importEnabled" value="true" id="rb03" <% if (isImportEnabled) { %>checked<% } %>> <label for="rb03">Yes</label>
            <input type="radio" name="importEnabled" value="false" id="rb04"<% if (!isImportEnabled) { %>checked<% } %>> <label for="rb04">No</label>
            </font>
        </td>
    </tr>
    <tr>
    	...
    	<td>
            <font size="-1">
            <input type="radio" name="exportEnabled" value="true" id="rb05"<% if (isExportEnabled) { %>checked<% } %>> <label for="rb05">Yes</label>
            <input type="radio" name="exportEnabled" value="false" id="rb06"<% if (!isExportEnabled) { %>checked<% } %>> <label for="rb06">No</label>
            </font>
        </td>
    </tr>
    <tr>
    ...
    <tr>
    	<td nowrap><font size="-1">Disable 24 hour buffer for importing messages:</font></td>
    	<td>
            <font size="-1">
            <input type="radio" name="disableCutOffDate" value="true" id="rb05"<% if (isCutOffDateDisabled) { %>checked<% } %>> <label for="rb05">Yes</label>
            <input type="radio" name="disableCutOffDate" value="false" id="rb06"<% if (!isCutOffDateDisabled) { %>checked<% } %>> <label for="rb06">No</label>
            </font>
    </tr>
    <tr>
    	...
    	<td valign="top"><textarea name="footer" cols="30" rows="5" wrap="virtual"><% if (footer!=null) { %><%=footer%><% } %></textarea></td>
    </tr>
    <tr>

...
<%!	// global variables/methods, etc
    ...
    private int getCurrentPreset(com.jivesoftware.util.Cache[] caches, int[][] cachePresetSizes) {
    ...
    if (setCacheSize) {
        if (cachePreset == CACHE_PRESET_CUSTOM) {
            ...
            for (int i=0; i<caches.length; i++) {
                com.jivesoftware.util.Cache cache = caches[i];
...
<tr bgcolor="#ffffff">
    <td width="50%">
    <font size="-1"><b><%= cache.getName() %></b></font>
    </td>
    <td width="25%">
    <%  if (custom) { %>
    <input type="text" name="custom<%= i %>K" size="6" value="<%= kFormat.format(totalMemK) %>" maxlength="6"><font size="-1">K</font>
    <%  } else { %>

...
<%  }
    ...
    while (i++ < range && forums.hasNext()) {
        ...
        boolean isMessageModOn = (
            forum.getModerationDefaultMessageValue() < JiveConstants.FORUM_MODERATION_VISIBLE
        );
%>
<tr bgcolor="#ffffff">
    ...
    <td width="96%">
        ...
        <%= forum.getName() %>
        <%  } else { %>
        <a href="pending-list.jsp?forum=<%= forum.getID() %>"><%= forum.getName() %></a>
        <%  } %>
        </font>
    </td>
    ...
    <td width="1%" nowrap>
        <font size="-1"></font>
        <font size="-1"><%= SkinUtils.formatDate(request,pageUser,forum.getModificationDate()) %></font>
    </td>
</tr>
<%  } %>
...
<tr>
<%  if (start > 0) { %>
	<td width="1%" nowrap><font size="-1"><a href="moderation.jsp?start=<%= start-range %>">Previous <%= range %> forums</a></font></td>
<%  } else { %>
...
<%  if (start+range < forumCount) { %>
	<td width="1%" nowrap><font size="-1"><a href="moderation.jsp?start=<%= start+range %>">Next <%= range %> forums</a></font></td>
<%  } else { %>
	<td width="1%" nowrap><font size="-1">&nbsp;</font></td>
<%  } %>
</tr>

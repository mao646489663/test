...
<%	// Get parameters
    ...
    if (toCategoryID != -1) {
        destCategory = forumFactory.getForumCategory(toCategoryID);
    }
    ...
    if (destCategory != null && !isSystemAdmin && !destCategory.isAuthorized(ForumPermissions.FORUM_CATEGORY_ADMIN)) {
        throw new UnauthorizedException("You don't have admin privileges to perform this operation.");
    }
...
<tr bgcolor="#ffffff">
    ...
    <td><select size="1" name="tocat">
        ...
        <%  for (Iterator iter=rootCategory.getRecursiveCategories(); iter.hasNext();) {
                ...
                // check permissions
                if (!isSystemAdmin && !cat.isAuthorized(ForumPermissions.FORUM_CATEGORY_ADMIN)) {
                    continue;
                }

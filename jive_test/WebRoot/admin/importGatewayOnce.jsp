...
<font size="-1">
<% if (gateway instanceof EmailGateway || gateway instanceof ImapGateway) { %>
...
<%  GatewayImporter importer = gateway.getGatewayImporter();
    ...
    <% } else if (importer instanceof ImapImporter)  { %>
    Jive is currently importing messages into forum <i><%=forum.getName()%></i> from account <%= ((ImapImporter) importer).getUsername() %>@<%= ((ImapImporter) importer).getHost() %>  <br>

...
<%	// Jive installation check. Load the Jive classes by reflection because
	...
	if (doSetup || configError) { %>
		<html>
		<head>
		<title>Jive Forums Professional - Admin</title>
		</head>
		<body>
        <font face="arial,helvetica,sans-serif" color="#333333">
        <b>Jive Forums Professional - Admin</b>
        </font>
        <ul>
...
<%		// for some reason, we have to call flush.. some app servers won't
    ...
    // License check
    com.jivesoftware.base.LicenseManager.validateLicense("Jive Forums Professional","3.0");

    // logout if requested:
	if ("true".equals(request.getParameter("logout"))) {
		try {
            List removables = new ArrayList(10);
            for (Enumeration enumm=session.getAttributeNames(); enumm.hasMoreElements();)
            {
                String attribName = (String)enumm.nextElement();
